...
Set `GIT_SSH_COMMAND` and `PUBLISH_CHECKOUT` in `/etc/snake-web.env` as shown in
[Install Snake Web]({% link pages/devops/install.md %}). The installed unit
...
`pages/status/index.md`, and commits only when that file changes. It never
force-pushes.
...
each changes solely the status page. A dirty checkout, unrelated pending commit,
